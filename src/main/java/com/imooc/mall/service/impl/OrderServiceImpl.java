...
public class OrderServiceImpl implements IOrderService {
    ...
    public ResponseVo<OrderVo> create(Integer uid, Integer shippingId) {
        ...
        //获取cartList离得products
        Set<Integer> productIdSet = cartList.stream()
                .map(Cart::getProductId).collect(Collectors.toSet());
        List<Product> productList=productMapper.selectByPrimaryKeys(productIdSet);
        Map<Integer,Product> map=productList.stream().
                collect(Collectors.toMap(Product::getId,product -> product));
        List<OrderItem> orderItemList=new ArrayList<>();
        Long orderNo=generateOrderNo();//企业用分布式唯一主键
        for(Cart cart:cartList){
            ...
            if(!ProductStatusEnum.ON_SELL.getCode().equals(product.getStatus())){
                return ResponseVo.error(ResponseEnum.PRODUCT_OFF_SELL_OR_DELETE,"商品不是在售状态."+product.getName());
            }
            //库存是否充足
